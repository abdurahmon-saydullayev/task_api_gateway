...
type Config struct {
	...
	BlogpostServiceHost string
	BlogpostServicePort string
...
func Load() Config {
	...
	config.BlogpostServiceHost = cast.ToString(getOrReturnDefaultValue("ORDER_SERVICE_HOST", "localhost"))
	config.BlogpostServicePort = cast.ToString(getOrReturnDefaultValue("ORDER_SERVICE_PORT", ":9092"))
